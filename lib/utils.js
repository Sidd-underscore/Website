...
// Global variable for contrast adjustment value
const CONTRAST_ADJUSTMENT = 200; // You can adjust this value as needed

export function adjustTextColor(hexColor) {
  // Remove the '#' if present
  hexColor = hexColor.replace("#", "");
  
  // Parse the hex color values for R, G, B
  ...
  // Helper function to adjust color brightness with global contrast value
  const adjustBrightness = (color, amount) => {
    return Math.min(255, Math.max(0, color + amount)); // Ensure values stay between 0 and 255
  };

  // Apply contrast adjustment based on luminance
  const contrastAdjustment = luminance > 0.5 ? -CONTRAST_ADJUSTMENT : CONTRAST_ADJUSTMENT;
